List<String> showsImages = [
  'https://image.tmdb.org/t/p/original/ztkUQFLlC19CCMYHW9o1zWhJRNq.jpg',
  'https://image.tmdb.org/t/p/original/ePXuKdXZuJx8hHMNr2yM4jY2L7Z.jpg',
  'https://image.tmdb.org/t/p/original/n8dKkbPkdEPFCFmjNvrUl1gUAYr.jpg',
  'https://image.tmdb.org/t/p/original/ApitDfnYDwFNOPfyVuhW5ElWzhe.jpg',
  'https://image.tmdb.org/t/p/original/kxwtTdHquKObHiT8LAWsHDQricj.jpg',
];
List<List<int>> showsGenresIds = [
  [18, 80],
  [80, 18, 53],
  [18, 9648],
  [99],
  [99, 10770],
];
List<String> showsTitle = [
  'Breaking Bad',
  'El Camino: A Breaking Bad Movie',
  'Breaking Bad Fortune Teller',
  'No Half Measures: Creating the Final Season of Breaking Bad',
  'Daniel Brühl: Breaking Bad',
];
List<String> showsRating = ['8.9', '6.9', '0', '8.4', '0'];
List<String> showsRatingCount = ['13213', '4609', '0', '108', '0'];

List<String> showsYear = ['2008', '2019', '2016', '2013', '2023'];
List<String> trailers = [
  'Sy6eNs3EW3E',
  'U2Qp5pL3ovA',
  'RlbR5N6veqw',
  '8g18jFHCLXk',
  'rNZ0xKaCdus',
];
List<String> peoplesImages = [
  'https://image.tmdb.org/t/p/original/pqdR8zqAWF87chGYlbdYr0YfC7g.jpg',
  'https://image.tmdb.org/t/p/original/eOh4ubpOm2Igdg0QH2ghj0mFtC.jpg',
  'https://image.tmdb.org/t/p/original/qYiaSl0Eb7G3VaxOg8PxExCFwon.jpg',
  'https://image.tmdb.org/t/p/original/niLc67b00QIa0oDWp47mXcv319e.jpg',
  'https://image.tmdb.org/t/p/original/cyWDj8CO7UeKyOnIY1Eu7KwADSO.jpg',
];
List<String> peoplesNames = [
  "Jeremy Piven",
  "Tom Cruise",
  "Sydney Sweeney",
  "Cameron Mitchell",
  "Brad Pitt",
];
