...
class HomeRemoteDataSourceImpl extends HomeRemoteDataSource {
  ...
  Future<List> getPicksForYou(int page) async {
    ...
    List result = [];
    (List<String>, List<String>) categorizedGenres = getCategorizedGenres(
      genres,
    );

    String genresParameterMovies = categorizedGenres.$1
        .map((id) => id.toString())
        .join('%7C');
    ...
    String genresParameterTvShow = categorizedGenres.$2
        .map((id) => id.toString())
        .join('%7C');
  ...
  Future<List<GenreModel>> getUserFavoriteGenres() async {
  ...
  Future<void> addShowToList(String listId, ShowMiniResultEntity show) async {
    await firebaseFirestore
        ...
        .doc(listId)
        .update({
          'shows': FieldValue.arrayUnion([show.toJson()]),
        });
  }
