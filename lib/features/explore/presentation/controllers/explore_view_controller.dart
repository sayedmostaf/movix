...
class ExploreViewController extends GetxController {
  ...
  Future<void> initMoviesSection() async {
    ...
    moviesExploreBanners.add(
      getBanners(nowPlayingMoviesController.movies.take(5).toList()),
    );
    moviesExplore.add(nowPlayingMoviesController.movies);

    moviesExploreBanners.add(
      getBanners(trendingMoviesController.movies.take(5).toList()),
    );
    moviesExplore.add(trendingMoviesController.movies);

    moviesExploreBanners.add(
      getBanners(popularMoviesController.movies.take(5).toList()),
    );
    moviesExplore.add(popularMoviesController.movies);

    moviesExploreBanners.add(
      getBanners(topRatedMoviesController.movies.take(5).toList()),
    );
