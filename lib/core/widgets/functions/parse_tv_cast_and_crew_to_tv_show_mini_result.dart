import 'package:movix/core/widgets/functions/enums.dart';
import 'package:movix/features/home/data/models/person_result/tv_cast.dart';
import 'package:movix/features/home/data/models/person_result/tv_crew.dart';
import 'package:movix/features/home/domain/entities/tv_show_mini_result_entity.dart';

List<TvShowMiniResultEntity>? parseTvCastAndCrewToTvShowMiniResult(
  List<TvCast>? cast,
  List<TvCrew>? crew,
) {
  List<TvShowMiniResultEntity> shows = [];
  for (var show in cast!) {
    shows.add(
      TvShowMiniResultEntity(
        id: show.id!,
        voteAverage: show.voteAverage,
        releaseDate: show.firstAirDate == null || show.firstAirDate!.isEmpty
            ? null
            : DateTime.parse(show.firstAirDate!),
        posterPath: show.posterPath,
        genres: show.genreIds,
        showType: ShowType.TV,
        name: show.name,
        voteCount: show.voteCount,
      ),
    );
  }
  for (var show in crew!) {
    shows.add(
      TvShowMiniResultEntity(
        id: show.id!,
        voteAverage: show.voteAverage,
        releaseDate: show.firstAirDate == null || show.firstAirDate!.isEmpty
            ? null
            : DateTime.parse(show.firstAirDate!),
        posterPath: show.posterPath,
        genres: show.genreIds,
        showType: ShowType.TV,
        name: show.name,
        voteCount: show.voteCount,
      ),
    );
  }
  return shows;
}
